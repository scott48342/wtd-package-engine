...
function packagesRouter({ packageEngineService }) {
  ...
  // Package tiles: wheels + a selected tire (with price) for the recommended size.
  // GET /v1/packages/bundles?vehicleId=...&targetDiameter=20&fitmentProfile=stock
  r.get('/bundles', async (req, res, next) => {
    try {
      const vehicleId = req.query.vehicleId ? String(req.query.vehicleId) : null;
      const vehicleModificationId = req.query.vehicleModificationId ? String(req.query.vehicleModificationId) : null;
      const targetDiameter = req.query.targetDiameter != null ? Number(req.query.targetDiameter) : null;
      ...
      const maxTireWidthDelta = req.query.maxTireWidthDelta != null ? Number(req.query.maxTireWidthDelta) : undefined;
      const wheelPageSize = req.query.wheelPageSize != null ? Number(req.query.wheelPageSize) : undefined;
      const tirePageSize = req.query.tirePageSize != null ? Number(req.query.tirePageSize) : undefined;

      const out = await packageEngineService.plusSizeBundles({
        vehicleId,
        vehicleModificationId,
        fitmentProfile,
        targetDiameter,
